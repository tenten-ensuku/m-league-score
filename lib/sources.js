...
function reconcile(kinma, official, { warnings = [] } = {}) {
  ...
  for (const name of new Set([...k.keys(), ...o.keys()])) {
    ...
    if (!a || !b || a.games !== b.games || !Number.isFinite(a.score) || !Number.isFinite(b.score) || Math.abs(a.score - b.score) > 0.01) {
      throw new Error(`Sources disagree: ${name}; Kinma PT=${a?.score}, games=${a?.games}; official PT=${b?.score}, games=${b?.games}; retaining the last verified data`);
    }
    if (a.games && Math.abs(a.avg - b.avg) > 0.02) {
      // A derived display statistic must not block verified points, but requires exact official counts.
      const counts = b.placements;
      if (!Array.isArray(counts) || counts.length !== 4 || counts.some(n => !Number.isInteger(n) || n < 0) || counts.reduce((sum, n) => sum + n, 0) !== b.games || Math.abs(counts.reduce((sum, n, i) => sum + n * (i + 1), 0) / b.games - b.avg) > 1e-9) {
        throw new Error(`Unverified official average: ${name}; retaining the last verified data`);
      }
      warnings.push(`${name}: 平均着順はキンマweb ${a.avg.toFixed(2)}、公式 ${b.avg.toFixed(2)}。PT・試合数は一致しているため、公式の着順回数から算出した値を採用しています。`);
    }
  }
